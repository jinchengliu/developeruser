...
-(void)addScroll1{
    ...
    for (int i =0; i<self.xl_buttonNames.count; i++) {
        ...
        [temp setBackgroundImage:[UIImage imageNamed:@"touzi_segment.png"] forState:UIControlStateNormal];
        [temp setBackgroundImage:[UIImage imageNamed:@"touzi_segment_f.png"] forState:UIControlStateSelected];
        temp.tag=i;
...
-(void)changed:(UIButton *)button{
    ...
    self.scroll2.delegate =self;
    if(self.delegate){
        [self.delegate XLScrollViewerScroll: button.tag];
    }
...
-(void)scrollViewWillBeginDragging:(UIScrollView *)scrollView{
    ...
    // 根据当前的x坐标和页宽度计算出当前页数
     current = floor((scrollView.contentOffset.x - pageWidth / 2) / pageWidth) + 1;
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if(current !=currentPage){
        ...
        if(self.delegate){
            [self.delegate XLScrollViewerScroll: currentPage];
        }
