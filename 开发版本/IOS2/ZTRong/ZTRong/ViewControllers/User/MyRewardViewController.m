...
@interface MyRewardViewController ()
{
    ...
    __weak IBOutlet UILabel *_sumMoney;
...
- (void)getNetworkData{
    ...
    if (![_selection isEqual:@"不限"]) {
        ...
        }else if ([_selection isEqual:@"近6个月"]){
            
            NSDate *start = [NSDate dateWithDaysBeforeNow:180];
            [dic setObject:[dateFormatter stringFromDate:start] forKey:@"startDate"];
        }
    ...
    [Tool ZTRPostRequest:[NSString stringWithFormat:@"%@%@",htmlUrl,kUserAwards] parameters:@{@"message":[StringHelper dictionaryToJson:params]} finishBlock:^(NSDictionary *dict) {
        ...
        if([dict[@"success"] intValue] == 1){
            ...
            _sumMoney.text = dict[@"map"][@"sumMoney"];
...
-(void)showPopo{
    ...
    if(_popup==nil){
        ...
        _popup.showDate =@[@"不限",@"近1个月",@"近3个月",@"近6个月"];
        
        __weak MyRewardViewController *blockself = self;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *userName = [NSString stringWithFormat:@"%@-%@",tempDict[@"regUserName"],tempDict[@"distributionLevl"]];
    ...
    cell.investID.text = tempDict[@"itemNum"];
    cell.orderMoney.text = [NSString stringWithFormat:@"￥%@",tempDict[@"orderAmountStr"]];
    cell.rate.text = tempDict[@"distributionRate"];
    cell.reward.text = [NSString stringWithFormat:@"￥%@",tempDict[@"operateAmountStr"]];
    if ([tempDict[@"createDate"] length]) {
        cell.dateTime.text = [tempDict[@"createDate"] componentsSeparatedByString:@" "][0];
    }
